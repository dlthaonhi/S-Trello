...
export const ProjectController = {
  async getAllProjects(req: AuthenticatedRequest, res: Response) {
    ...
    try {
      const serviceResponse = await ProjectService.getAllProjetcs(userId);
      handleServiceResponse(serviceResponse, res);
    } catch (error) {
      const errorMessage = `Error getting project: ${(error as Error).message}`;
  ...
  async createBoardByTemplateId(req: AuthenticatedRequest, res: Response) {
    const userId: string | any = req.id;
    const projectId: string | any = req.params.projectId;
    const templateId: string | any = req.params.templateId;
    const boardData: Boards = req.body;
    try {
      const serviceResponse = await ProjectService.createBoardFromTemplate(userId, templateId, projectId, boardData);
      handleServiceResponse(serviceResponse, res);
    } catch (error) {
      const errorMessage = `Error creating board by template ID: ${(error as Error).message}`;
      res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({
        status: ResponseStatus.Failed,
        message: errorMessage,
        data: null,
