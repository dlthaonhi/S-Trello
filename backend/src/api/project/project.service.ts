...
export const ProjectService = {
  getAllProjetcs: async (userId: string): Promise<ServiceResponse<Projects[] | null>> => {
    try {
      ...
      const gettedAllProjects = await projectRepository.findAllAsync();
      if (!gettedAllProjects) {
        return new ServiceResponse(
          ResponseStatus.Failed,
      ...
      return new ServiceResponse<Projects[]>(
        ...
        "All projects've getted successfully!",
        gettedAllProjects,
        StatusCodes.CREATED
      );
    } catch (ex) {
      const errorMessage = `Error getting project: ${(ex as Error).message}`;
      return new ServiceResponse(
        ResponseStatus.Failed,
        errorMessage,
        null,
  ...
  deleteProject: async (id: string): Promise<ServiceResponse<null>> => {
    try {
  ...
  restoreProject: async (id: string): Promise<ServiceResponse<null>> => {
    try {
  ...
  createBoardFromTemplate: async(userId: string, templateId: string, projectId: string, boardData: Boards): Promise<ServiceResponse<Boards | null>> => {
    try {
