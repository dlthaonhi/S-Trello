...
export const cardRepository = dataSource.getRepository(Cards).extend({
  ...
  async findByIdAsync(cardId: string): Promise<Cards | null> {
    console.log(cardId)
    return await cardRepository.findOne({
      where: { id:cardId, deletedAt: IsNull(), isTemplate: false },
  });    
    // return await cardRepository.findOne({ id: id,deletedAt: IsNull(), isTemplate: false });
  },
  ...
  async findByListIdAndPositionAsync(listId: string, position: number): Promise<Cards | null> {
    return this.findOne({
      where: {  listID: {id: listId}, position: position, deletedAt: IsNull(), isTemplate: false }
    });
  },

  async countCardsByListIdAsync(listId: string): Promise<number> {
    return this.count({ where: {listID: {id: listId}, deletedAt: IsNull(), isTemplate: false}})
  },
...
export const cardMemberRepository = dataSource.getRepository(CardMembers).extend({
    ...
    async findAllByCardIdAsync(cardId: string): Promise<CardMembers[]> {
      return this.find({
          where: { cardID: { id: cardId, deletedAt: IsNull() }}
      });
    },
    ...
    async findByCardIdAsync(cardId: string): Promise<CardMembers | null> {
      return this.findOneBy({ cardID: { id: cardId, deletedAt: IsNull() } });
    },
    ...
    async findByCardAndUserIdAsync(cardId: string, userId: string): Promise<CardMembers | null> {
      return this.findOne({
        where: { cardID: { id: cardId, deletedAt:IsNull() }, userID: { id: userId, deletedAt: IsNull() } },
      });
    },
