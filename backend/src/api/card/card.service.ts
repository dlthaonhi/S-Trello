...
export const CardService = {
  getAllCards: async():Promise<ServiceResponse<Cards[] | null>> => {
    try {
      const cards = await cardRepository.findAllAsync();
      if (!cards) {
      ...
      return new ServiceResponse<Cards[]>(
        ...
        "Cards getted successfully!",
        cards,
        StatusCodes.CREATED
      );
    } catch (ex) {
      const errorMessage = `Error getting card: ${(ex as Error).message}`;
      return new ServiceResponse(
        ResponseStatus.Failed,
        errorMessage,
        null,
  ...
  deleteCard: async (id: string): Promise<ServiceResponse<null>> => {
    try {
  ...
  restoreCard: async(id: string): Promise<ServiceResponse<null>> => {
    try {
