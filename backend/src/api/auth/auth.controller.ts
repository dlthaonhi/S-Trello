...
export const AuthController = {
  ...
  async handleGoogleRes(req: Request, res: Response){
    try {
      let {code} = req.query;
      if (!code || typeof code !== 'string') throw new Error('Code: Not found');
      const serviceResponse = await authService.setTokenGoogleLogin(code);
      console.log(`Token: ${serviceResponse.data}`);
      handleServiceResponse(serviceResponse, res);
    } catch (error) {
      const errorMessage = `Error handling Google response: ${(error as Error).message}`;
      res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({
        status: ResponseStatus.Failed,
        message: errorMessage,
        data: null,
