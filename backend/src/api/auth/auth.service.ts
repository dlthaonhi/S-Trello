...
export const authService = {
  ...
  setTokenGoogleLogin: async(code: string): Promise<ServiceResponse<string | null>> => {
    try {
        const accessTokenData = await oAuthGoogleService.getTokenData(code) as { id_token: string, access_token: string };

        const dataInfo = await oAuthGoogleService.getDataFromToken(accessTokenData)
        const { email, name } = dataInfo
        console.log(`Data: ${JSON.stringify(dataInfo)}`);
        console.log(email,name);
        

        //Data: {"iss":"https://accounts.google.com","azp":"742965807795-u0t6moip0pkuktota6h98f0690lvcep4.apps.googleusercontent.com","aud":"742965807795-u0t6moip0pkuktota6h98f0690lvcep4.apps.googleusercontent.com","sub":"114700979713120849768","email":"trangnguyenthi220505@gmail.com","email_verified":"true","at_hash":"kpezJUId4uxyPA47j44r6Q","iat":"1739285111","exp":"1739288711","alg":"RS256","kid":"eec534fa5b8caca201ca8d0ff96b54c562210d1e","typ":"JWT"}
        
        let user = await userRepository.findByEmailAsync(email)
        if (!user) {
            // user = await userRepository.createUserAsync({...accessTokenData, ...email, name: 'Unknown Name' })
            user = await userRepository.createUserAsync({...accessTokenData, email, name})
            if (!user) {
              return new ServiceResponse(
                ResponseStatus.Failed,
                "Error creating new user from Google data infomation",
                null,
                StatusCodes.BAD_REQUEST
              );
            }
        } else {
            // await userRepository.updateUserByIdAsync(user.id, accessTokenData)
            console.log("Updating...");
        }
        console.log(`User: `, user);


        let token = await generateJwt({ userId: user.id} );
        if (!token) {
          return new ServiceResponse(
            ResponseStatus.Failed,
        ...
        return new ServiceResponse(
          ResponseStatus.Success,
          "Success setting token for Google login",
          token,
          StatusCodes.OK
        );
    } catch (error: any) {
      return new ServiceResponse(
        ResponseStatus.Failed,
        `Error setting token for Google login: ${error.message}`,
        null,
        StatusCodes.INTERNAL_SERVER_ERROR
      );
    }
